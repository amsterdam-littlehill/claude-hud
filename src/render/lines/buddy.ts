...
function buddyVisualLength(str: string): number {
...
export function renderBuddyColumn(detailLevel: BuddyDetailLevel = 'full'): BuddyColumn | null {
  ...
  if (detailLevel === 'compact') {
    const spriteLines = renderSprite(companion, spriteFrame).map(l =>
      blink ? l.replaceAll(companion.eye, '-') : l,
    );
    const face = blink
      ? renderFace(companion).replaceAll(companion.eye, '-')
      : renderFace(companion);
  ...
  const spriteLines = renderSpriteDepth(companion, spriteFrame, Date.now()).map(l =>
    blink ? l.replaceAll(companion.eye, dim('-')) : l,
  );
...
export function renderBuddyLine(): string | null {
  const state = ensureBuddyState();
  if (!state) return null;
  ...
  const lines = renderSprite(companion, spriteFrame).map(l =>
    blink ? l.replaceAll(companion.eye, '-') : l,
  );
