import { axolotl, blob, cactus, capybara, cat, chonk, dragon, duck, ghost, goose, mushroom, octopus, owl, penguin, rabbit, robot, snail, turtle, } from './types.js';
...
function spriteNormal(plain, x, y, ch, bounds) {
...
function blinnPhong(normal, light, view) {
...
function rotatingLight(t) {
    const angle = t * 0.6; // slow orbit
    return [Math.cos(angle), Math.sin(angle), 0.55];
}
/** Render a sprite with spherical normal 3-D shading. */
export function renderSpriteDepth(bones, frame = 0, timeMs = Date.now()) {
    const frames = BODIES[bones.species];
    const plain = frames[frame % frames.length].map(line => line.replaceAll('{E}', bones.eye));
    ...
    if (!plain[0].trim() && frames.every(f => !f[0].trim()))
        result.shift();
    return result;
}
